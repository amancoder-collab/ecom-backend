...
export class SubscribeService {
  ...
  async subscribe(dto: SubEmailDto) {
    try {
      return await this.prismaService.subscribe.upsert({
  ...
  async list(page: number, limit: number, type: ListOrder, sellerId: string) {
    ...
    const result = await this.prismaService.subscribe.findMany({
      skip,
      take,
      orderBy: {
        createdAt: type,
  ...
  async remove(id: string, sellerId: string) {
    ...
    return await this.prismaService.subscribe.delete({
      where: { id },
    });
  }
}
